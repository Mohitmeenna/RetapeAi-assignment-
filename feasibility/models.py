...
def round_half_up(value) -> int:
    """Round to the nearest integer, with ``.5`` always rounding away from zero.

    The spec requires explicit round-half-up; Python's built-in ``round`` uses
    banker's rounding (half-to-even), so we implement it via ``Decimal``.
    """
    return int(Decimal(str(value)).quantize(Decimal("1"), rounding=ROUND_HALF_UP))


def offer_total_cents(offer: Offer) -> int:
    return round_half_up(Decimal(str(offer.settlement_pct)) * Decimal(offer.current_balance_cents))


def program_fee_cents(offer: Offer, rules: CreditorRules) -> int:
    return round_half_up(Decimal(str(rules.program_fee_pct)) * Decimal(offer.original_balance_cents))
